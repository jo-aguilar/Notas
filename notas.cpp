...
void cria_boletim(){
	...
	** criar verificação de integridade de entrada a ser gravada, não 
	   permitindo entrada de um boletim vazio, por exemplo*/
	...
	std::cin >> nome_doc;
	std::ofstream ofs = std::ofstream((nome_doc + ".txt").c_str());	
...
void manipula_boletim(){
	...
	boletins já criados 
	** função de remoção ainda precisa ser criada*/
	...
	std::cin >> arquivo;
	ifs = std::ifstream((arquivo + ".txt").c_str());
	if(ifs.fail()){
...
void refaz_boletim(std::string nome, std::vector<Alvo>alvos){
	...
	std::ofstream ofs = std::ofstream((nome + ".txt").c_str());
	for(int clk = 0; clk < alvos.size(); clk++){
...
void mostra_lista_boletim(){
	...
	*Um diretório exclusivo para os boletins precisa ser feito */
	DIR* dir;
...
void remove_boletim(){
	...
	std::cout << ">>";
	std::cin >> arquivo;
	if(remove((arquivo +".txt").c_str())==false)
